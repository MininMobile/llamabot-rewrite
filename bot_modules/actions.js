...
Actions.AddCommand("pisson", (message, args, bot) => {
	...
	message.channel.send(`${message.author} pissed on ${args[1]}, right in their *smug* fucking face.`).catch(console.error);
});
...
Actions.AddCommand("kiss", (message, args, bot) => {
	...
	message.channel.send(`${message.author} smooched ${args[1]}, crime rates have gone down 50%!`).catch(console.error);
});

Actions.AddCommand("pat", (message, args, bot) => {
	if (!vm(args)) return message.reply("invalid arguments, please mention the user correctly.");

	message.channel.send(`${message.author} pat ${args[1]} on their head, who's a good boy?`).catch(console.error);
});
